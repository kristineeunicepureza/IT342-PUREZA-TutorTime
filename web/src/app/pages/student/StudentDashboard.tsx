...
export function StudentDashboard() {
  ...
  useEffect(() => {
    api.get<Booking[]>('/api/bookings/my-bookings')
      .then(data => {
        const active    = data.filter(b => b.status === 'PENDING' || b.status === 'CONFIRMED').length;
        const completed = data.filter(b => b.status === 'COMPLETED').length;
        setStats({ active, completed, hours: completed });
      })
      .catch(() => {});
  }, []);
  ...
  const statCards = [
    { label: 'Active Bookings',    value: stats.active,    icon: Calendar,    color: '#3B82F6' },
    { label: 'Completed Sessions', value: stats.completed, icon: TrendingUp,  color: '#10B981' },
    { label: 'Hours Learned',      value: stats.hours,     icon: Clock,       color: '#F59E0B' },
  ];
    ...
    <div className="flex min-h-screen" style={{ background: 'linear-gradient(160deg, #f0f6ff 0%, #e8f0fe 60%, #EFF6FF 100%)' }}>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '28px 32px' }}>
          ...
          {/* Hero */}
          <div style={{ background: 'linear-gradient(135deg, #001a4d 0%, #002F6C 45%, #0047AB 100%)', borderRadius: '20px', padding: '28px 32px', marginBottom: '28px', position: 'relative', overflow: 'hidden' }}>
          ...
          {/* Account Details */}
          <div style={{ marginTop: '24px', background: 'white', borderRadius: '16px', padding: '24px', boxShadow: '0 4px 20px rgba(0,47,108,0.07)', border: '1px solid rgba(59,130,246,0.08)' }}>
