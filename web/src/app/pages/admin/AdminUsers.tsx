    ...
    <DashboardLayout role="ADMIN">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '14px', marginBottom: '24px' }}>
        {(['STUDENT','TUTOR','ADMIN'] as const).map(r => {
          ...
          return (
            <div key={r} style={{ background: 'white', borderRadius: '14px', padding: '18px 20px', display: 'flex', alignItems: 'center', gap: '14px', boxShadow: '0 4px 16px rgba(0,47,108,0.07)', border: '1px solid rgba(59,130,246,0.08)' }}>
      ...
      <div style={{ background: 'white', borderRadius: '16px', boxShadow: '0 4px 20px rgba(0,47,108,0.07)', border: '1px solid rgba(59,130,246,0.08)', overflow: 'hidden' }}>
        {/* Toolbar */}
        <div style={{ padding: '16px 20px', borderBottom: '1px solid #e0eaff', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '12px' }}>
          <div style={{ position: 'relative', flex: 1, maxWidth: '340px' }}>
            ...
            <input
              ...
              style={{ width: '100%', padding: '9px 12px 9px 36px', borderRadius: '10px', border: '1.5px solid #e0eaff', background: '#f7faff', fontSize: '13.5px', outline: 'none', color: '#001a4d', boxSizing: 'border-box' }}
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={3} style={{ padding: '40px', textAlign: 'center', color: '#94a3b8' }}>
                  <Users size={40} style={{ margin: '0 auto 10px', opacity: 0.4 }} /><br />No users found.
                </td></tr>
              ) : filtered.map((u, i) => {
                const Icon = roleIcon[u.role];
                return (
                  <tr key={u.id} style={{ borderBottom: i < filtered.length-1 ? '1px solid #f0f4fa' : 'none' }}
                  ...
                  >
                    <td style={{ padding: '14px 20px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                        ...
                        <div>
                          <div style={{ color: '#001a4d', fontWeight: 600, fontSize: '14px' }}>{u.firstName} {u.lastName}</div>
