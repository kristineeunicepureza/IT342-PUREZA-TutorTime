...
interface Booking     { status: string; }
interface UserProfile { id: number; role: string; }
...
export function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      api.get<Booking[]>('/api/bookings/all'),
      api.get<UserProfile[]>('/api/admin/tutors/pending'),
    ]).then(([users, bookings, pendingTutors]) => {
      const totalUsers     = users.filter(u => u.role !== 'ADMIN').length;
      const activeSessions = bookings.filter(b => b.status === 'CONFIRMED').length;
      const pendingReviews = pendingTutors.length;
      setStats({ totalUsers, activeSessions, pendingReviews });
    }).catch(() => {});
  }, []);
  ...
  const quickActions = [
    { icon: BookOpen, label: 'Manage Subjects',  desc: 'Add or edit subjects',  path: '/admin/manage-subjects',  gradient: 'linear-gradient(135deg, #3B82F6, #1D4ED8)', shadow: 'rgba(37,99,235,0.3)'  },
    { icon: MapPin,   label: 'Manage Locations', desc: 'Add or edit locations', path: '/admin/manage-locations', gradient: 'linear-gradient(135deg, #10B981, #059669)', shadow: 'rgba(5,150,105,0.3)'  },
    { icon: Users,    label: 'Manage Users',      desc: 'View all users',        path: '/admin/users',            gradient: 'linear-gradient(135deg, #F59E0B, #D97706)', shadow: 'rgba(217,119,6,0.3)'  },
  ];
  ...
  return (
    <div className="flex min-h-screen" style={{ background: 'linear-gradient(160deg, #f0f6ff 0%, #e8f0fe 60%, #EFF6FF 100%)' }}>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        {/* Top bar */}
        <div style={{ background: 'rgba(255,255,255,0.85)', backdropFilter: 'blur(12px)', borderBottom: '1px solid rgba(59,130,246,0.1)', padding: '14px 32px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', boxShadow: '0 2px 12px rgba(0,47,108,0.06)' }}>
          <div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', padding: '5px 12px', borderRadius: '20px', background: 'linear-gradient(135deg, #DBEAFE, #EFF6FF)', border: '1px solid #BFDBFE' }}>
              <ShieldCheck size={13} color="#2563EB" />
              <span style={{ color: '#1D4ED8', fontWeight: 700, fontSize: '12px' }}>Admin</span>
            </div>
            <div style={{ width: '36px', height: '36px', borderRadius: '50%', background: 'linear-gradient(135deg, #60A5FA, #2563EB)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 3px 10px rgba(59,130,246,0.35)', cursor: 'pointer' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '28px 32px' }}>
          ...
          {/* Hero */}
          <div style={{ background: 'linear-gradient(135deg, #001a4d 0%, #002F6C 40%, #003fa0 80%, #0052cc 100%)', borderRadius: '20px', padding: '28px 32px', marginBottom: '28px', position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px', marginBottom: '28px' }}>
            {statCards.map(({ label, value, icon: Icon, color }) => (
              <div key={label} style={{ background: 'white', borderRadius: '16px', padding: '20px', boxShadow: '0 4px 20px rgba(0,47,108,0.07)', border: '1px solid rgba(59,130,246,0.08)', display: 'flex', alignItems: 'center', gap: '16px' }}>
          ...
          {/* Admin Controls */}
          <h2 style={{ color: '#001a4d', fontWeight: 800, fontSize: '17px', marginBottom: '14px', letterSpacing: '-0.3px' }}>Admin Controls</h2>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '14px', marginBottom: '24px' }}>
            {quickActions.map(({ icon: Icon, label, desc, path, gradient, shadow }) => (
              <button key={path} onClick={() => navigate(path)}
                style={{ background: 'white', borderRadius: '16px', padding: '22px 20px', border: '1px solid rgba(59,130,246,0.1)', boxShadow: '0 4px 18px rgba(0,47,108,0.07)', cursor: 'pointer', transition: 'all 0.25s', textAlign: 'left', display: 'flex', flexDirection: 'column', gap: '14px' }}
                onMouseEnter={e => { e.currentTarget.style.transform = 'translateY(-4px)'; e.currentTarget.style.boxShadow = `0 12px 32px ${shadow}`; e.currentTarget.style.borderColor = '#93C5FD'; }}
                onMouseLeave={e => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = '0 4px 18px rgba(0,47,108,0.07)'; e.currentTarget.style.borderColor = 'rgba(59,130,246,0.1)'; }}
              >
