...
export function TutorDashboard() {
  ...
  const [stats, setStats] = useState({ upcoming: 0, completed: 0, hours: 0 });
  ...
  useEffect(() => {
    api.get<Booking[]>('/api/bookings/tutor-bookings')
      .then(data => {
        const upcoming  = data.filter(b => b.status === 'CONFIRMED' || b.status === 'PENDING').length;
        const completed = data.filter(b => b.status === 'COMPLETED').length;
        setStats({ upcoming, completed, hours: completed });
      })
      .catch(() => {});
  }, []);
  ...
  const statCards = [
    { label: 'Upcoming Sessions',  value: stats.upcoming,  icon: Calendar,   color: '#3B82F6' },
    { label: 'Completed Sessions', value: stats.completed, icon: TrendingUp, color: '#10B981' },
    { label: 'Hours Taught',       value: stats.hours,     icon: Clock,      color: '#F59E0B' },
  ];
    ...
    <div className="flex min-h-screen" style={{ background: 'linear-gradient(160deg, #f0f6ff 0%, #e8f0fe 60%, #EFF6FF 100%)' }}>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div style={{ background: 'rgba(255,255,255,0.85)', backdropFilter: 'blur(12px)', borderBottom: '1px solid rgba(59,130,246,0.1)', padding: '14px 32px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', boxShadow: '0 2px 12px rgba(0,47,108,0.06)' }}>
          ...
          <div
            style={{ width: '36px', height: '36px', borderRadius: '50%', background: 'linear-gradient(135deg, #60A5FA, #2563EB)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 3px 10px rgba(59,130,246,0.35)', cursor: 'pointer' }}
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '28px 32px' }}>
          ...
          {/* Hero */}
          <div style={{ background: 'linear-gradient(135deg, #1a2a6c 0%, #002F6C 40%, #0047AB 80%, #005bbf 100%)', borderRadius: '20px', padding: '28px 32px', marginBottom: '28px', position: 'relative', overflow: 'hidden' }}>
            ...
            <div style={{ position: 'relative' }}>
              ...
              <button
                ...
                style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', padding: '10px 20px', borderRadius: '10px', background: 'linear-gradient(135deg, #3B82F6, #1D4ED8)', color: 'white', fontWeight: 700, fontSize: '13.5px', border: 'none', cursor: 'pointer', boxShadow: '0 4px 14px rgba(37,99,235,0.45)', transition: 'all 0.2s' }}
          ...
          {/* Quick Actions */}
          <h2 style={{ color: '#001a4d', fontWeight: 800, fontSize: '17px', marginBottom: '14px', letterSpacing: '-0.3px' }}>Quick Actions</h2>
